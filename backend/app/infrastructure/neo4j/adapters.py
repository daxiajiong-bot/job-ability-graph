        ...
        *,
        ...
        WHERE $snapshot_id IN coalesce(seed.graph_ids, [])
          AND (
            ...
            OR (
              size($seed_ids) = 0
              AND size($query_terms) > 0
              AND any(term IN $query_terms WHERE any(value IN [
                coalesce(seed.name, ""),
                coalesce(seed.title, ""),
                coalesce(seed.id, ""),
                coalesce(seed.type, ""),
                coalesce(seed.document_type, "")
              ] WHERE toLower(toString(value)) CONTAINS term))
            )
            OR (size($seed_ids) = 0 AND size($query_terms) = 0)
          )
        WITH DISTINCT seed
        LIMIT $seed_limit
...
def _query_terms(query: str) -> list[str]:
    """Extract useful bilingual entity terms from a natural-language question."""
    text = query.strip().lower()
    if not text:
        return []

    terms = re.findall(r"[a-z0-9+#.]{2,}", text)
    stop_words = {"哪些", "什么", "如何", "需要", "要求", "相关", "之间", "以及", "可以", "岗位"}
    for chunk in re.findall(r"[\u4e00-\u9fff]+", text):
        for stop_word in stop_words:
            chunk = chunk.replace(stop_word, " ")
    ...
    return list(dict.fromkeys(term for term in terms if len(term) >= 2))[:20]
