"""LLM-reserved resume extraction helpers."""
...
from backend.app.llm.prompts import RESUME_EXTRACTION_PROMPT


def extract_resume_with_llm(text: str) -> Dict[str, Any]:
    availability = is_llm_available()
    if not availability["available"]:
        return {"llm_used": False, **availability}
    return call_llm(RESUME_EXTRACTION_PROMPT.replace("{resume_text}", text), json_mode=True)


def merge_resume_profiles(rule_profile: Dict[str, Any], llm_profile: Optional[Dict[str, Any]]) -> Dict[str, Any]:
