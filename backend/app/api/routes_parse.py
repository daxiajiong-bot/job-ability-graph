...
def parse_jd_endpoint(payload: JDParseRequest) -> JDParseResponse:
    try:
        result = parse_jd_profile(_payload_text(payload, "jd_text"), use_llm=payload.use_llm, save_artifacts=payload.save_artifacts)
...
def parse_resume_endpoint(payload: ResumeParseRequest) -> ResumeParseResponse:
    try:
        result = parse_resume_profile(_payload_text(payload, "resume_text"), use_llm=payload.use_llm, save_artifacts=payload.save_artifacts)
    except HTTPException:
        raise
    except Exception as exc:
...
def parse_resume_document_endpoint(payload: ResumeDocumentParseRequest) -> ResumeDocumentParseResponse:
    ...
    try:
        ...
        result = parse_resume_profile(document_text.text, use_llm=payload.use_llm, save_artifacts=payload.save_artifacts)
