...
```json
{
  "schema_version": "profile-extraction/v2",
  "document_type": "resume",
  "candidate": {"name": null, "current_title": null, "years_of_experience": null},
  "career_intent": {"target_position": null},
  "education": [],
  "work_experience": [],
  "project_experience": [],
  "skills": [],
...
JD 抽取对应 `JDProfile`，包含 `job`、`company`、`employment`、`responsibilities`、`requirements`、`skills`、`capabilities`、`application_scenarios`、`evaluation_signals` 和 `evidence`。创建画像后，解析产物会落盘到 `data/structured/profiles/`，接口返回 `profile.artifacts.profile_json`。
...
3. **技能归一化与画像**：优先使用官方 ESCO 快照索引做概念对齐，不维护手写技能词表。
