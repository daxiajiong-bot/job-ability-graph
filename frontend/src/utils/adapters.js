...
export function adaptGraphForECharts(graphNodes, graphEdges) {
  ...
  const nodes = graphNodes
    ...
    .map((node) => ({
      name: node.properties?.title || node.properties?.name || node.name || node.node_id,
      category: node.category ?? categoryMap[node.label] ?? 1,
      symbolSize: node.label === "Job" ? 45 : node.label === "Company" ? 30 : 20,
      value: node.label,
      id: node.node_id,
    }));
  ...
  const links = graphEdges
    .filter((e) =>
      ["REQUIRES_SKILL", "REQUIRES_TECHNOLOGY", "HAS_KNOWLEDGE", "HAS_ABILITY", "BELONGS_TO_CAPABILITY"].includes(e.relation_type || e.type)
    )
    .map((edge) => ({
