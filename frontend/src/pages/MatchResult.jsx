...
// ── 等待画像任务完成的轮询辅助函数 ──
function waitForProfileTask(taskId, pollFn, maxWaitMs = 600000) {
  return new Promise((resolve, reject) => {
    const startTime = Date.now();
    const interval = setInterval(async () => {
      if (Date.now() - startTime > maxWaitMs) {
        clearInterval(interval);
        reject(new Error("画像生成超时"));
        return;
      }
      try {
        ...
        if (task.status === "succeeded") {
          clearInterval(interval);
          resolve({
            profileId: task.profile?.id || task.profile?.profile_id || task.profile_id,
            profile: task.profile || null,
          });
        } else if (task.status === "failed") {
          clearInterval(interval);
          reject(new Error(task.error || "画像生成失败"));
        }
      } catch (e) {
        clearInterval(interval);
        reject(e);
...
export default function MatchResult() {
  ...
  const {
    addMatchHistory, initUserId,
    lastMatchInputs, lastMatchResult, setLastMatch,
    setCachedCandidateProfile, setCachedJobProfile,
    settings,
  } = useStore();

  // 从缓存恢复上次匹配的输入和结果
  const [resumeText, setResumeText] = useState(lastMatchInputs?.resumeText || "");
  const [jdText, setJdText] = useState(lastMatchInputs?.jdText || "");
  const [matchResult, setMatchResult] = useState(lastMatchResult || null);
  ...
  async function handleMatch() {
    ...
    try {
      ...
      const resumeDocId = resumeDocRes.data.data.document.id;
      ...
      const jdDocId = jdDocRes.data.data.document.id;

      message.loading({ content: "正在生成候选人画像（约30秒）...", key: "match" });
      // Ollama serves one local model; serialize the two large extraction
      // requests so they do not queue behind each other and time out.
      const candTaskRes = await createCandidateProfile(resumeDocId);
      const candTaskId = candTaskRes.data.data.task.task_id;
      if (!candTaskId) throw new Error("候选人画像任务创建失败：未返回任务 ID");
      const candProfile = await waitForProfileTask(candTaskId, getCandidateProfileTask);

      const jobTaskRes = await createJobProfile(jdDocId);
      const jobTaskId = jobTaskRes.data.data.task.task_id;
      if (!jobTaskId) throw new Error("岗位画像任务创建失败：未返回任务 ID");
      const jobProfile = await waitForProfileTask(jobTaskId, getJobProfileTask);
      const candProfileId = candProfile.profileId;
      const jobProfileId = jobProfile.profileId;
      if (!candProfileId || !jobProfileId) {
        throw new Error("Profile task completed without returning a profile id");
      }
      // 缓存候选人画像
      if (candProfile.profile) {
        setCachedCandidateProfile(resumeDocId, candProfile.profile);
      }

      message.loading({ content: "正在生成岗位画像（约30秒）...", key: "match" });
      // 缓存岗位画像
      if (jobProfile.profile) {
        setCachedJobProfile(jdDocId, jobProfile.profile);
      }
      ...
      if (matchData?.state === "error") {
        throw new Error(matchData.summary || matchData.warnings?.[0] || "匹配服务暂时不可用");
      }
      setMatchResult(matchData);

      // 缓存匹配结果和输入
      setLastMatch({ resumeText, jdText }, matchData);
  ...
  function getRadarOption() {
    ...
    return {
      tooltip: {
        trigger: "item",
        backgroundColor: settings.theme === "dark" ? "rgba(13, 17, 23, 0.95)" : "rgba(255, 255, 255, 0.95)",
        borderColor: settings.theme === "dark" ? "rgba(77, 214, 255, 0.2)" : "rgba(0, 0, 0, 0.1)",
        textStyle: { color: settings.theme === "dark" ? "#e6edf3" : "#1f1f1f" },
        formatter: (params) => {
      ...
      radar: {
        ...
        axisName: {
          color: settings.theme === "dark" ? "#8b949e" : "#666",
          fontSize: 12,
          fontWeight: 500,
        },
      },
  ...
  function handleExportReport(format = "text") {
    ...
    switch (format) {
      case "json":
        exportReportAsJSON(matchResult, null);
        break;
      case "csv":
        exportSkillsAsCSV(matchResult);
        break;
      default:
        exportReportAsText(matchResult, null);
    }
  ...
  return (
    <div>
      ...
      <Spin spinning={loading}>
        ...
        {/* AI 学习建议 */}
        {matchResult && <LearningAdvice matchResult={matchResult} />}
